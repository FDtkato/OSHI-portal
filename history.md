...
#### SNSハンドル修正（DOM調査で判明）
- X: 公式サイトDOMには `tigerlee0620` がリンクされていたが、正しいハンドルは `tigerlee__info`
  - `tigerlee__info` → `tigerlee0620` → `tigerlee__info` に再修正
- Instagram: 公式サイトDOMには `tgl___official` がリンクされていたが、実際にアクセスすると「プロフィールは利用できません」と表示（存在しないアカウント）
  - 正しいハンドルは `tigerlee__official`（プロフィール存在を確認済み）
  - `tigerlee__official` → `tgl___official` → `tigerlee__official` に再修正
...
- [x] GitHub Actions ビルド & GitHub Pages デプロイ成功

### 7. GitHub Pages デプロイ

- リポジトリ: https://github.com/FDtkato/OSHI-portal
- サイトURL: https://FDtkato.github.io/OSHI-portal/
- `astro.config.mjs`: `site: "https://FDtkato.github.io"`, `base: "/OSHI-portal"`
- GitHub Actions ワークフロー: `environment: github-pages` 追加、Deployステップに `id: deploy` 追加
- Node.js: `20` → `22` に変更（Astro v6 が `>=22.12.0` を要求）

### 8. ニュース表示改善

- ニュースを最新順にソート（`date` の降順比較）
- 4件目以降はスクロール表示（`max-h-[24rem] overflow-y-auto`）
- Xタイムラインは `data-height="480"` で既にスクロール対応済み
- Instagramはプロフィールリンクカード1枚のみのためスクロール不要

### 9. .sfdx 誤コミット修正

- `git add -A` で無関係な `.sfdx/`（Salesforce DX）フォルダが意図せずコミットされた
- `.gitignore` に `.sfdx/` を追加し、`git rm -r --cached .sfdx` で追跡解除

### 10. 残課題

- [x] ~~**タイガーリー DOM 調査**: ryzm.jp のSPA DOM構造を調査し、Puppeteerスクリプトのセレクタを修正する~~
- [x] ~~**SNSハンドル確認**: X (`tigerlee__info`), Instagram (`tigerlee__official`) に修正済み~~
- [x] ~~**GitHub リポジトリ作成 & デプロイ**: GitHub Pages デプロイ成功~~
- [x] ~~**astro.config.mjs base設定**: `base: "/OSHI-portal"` 設定済み~~
